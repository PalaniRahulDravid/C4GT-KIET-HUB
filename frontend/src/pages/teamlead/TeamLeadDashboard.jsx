...
export default function TeamLeadDashboard() {
  ...
  const handleCreateAndAttachCustomLink = async () => {
    if (!customLinkTitle.trim() || !customLinkUrl.trim()) {
      showToast('Please provide both Resource Title and URL', 'error');
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.success && data.resource) {
        setHubResources((prev) => [data.resource, ...prev]);
        setSelectedResourceIds((prev) => [...prev, data.resource._id]);
        setCustomLinkTitle('');
        setCustomLinkUrl('');
        setCustomLinkType('link');
        setShowAddCustomLink(false);
        showToast('Resource link created and attached! ✓');
      } else {
        showToast(data.message || 'Failed to create resource link', 'error');
      }
    } catch (err) {
      console.error('Error creating resource link:', err);
      showToast('Error creating resource link', 'error');
    } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateTask} className="space-y-4">
              ...
              {/* Attach Learning Resources for Students */}
              <div className="space-y-2.5 pt-3 border-t border-slate-100">
                <div className="flex items-center justify-between">
                  <div>
                    <label className="text-xs font-semibold text-slate-800 flex items-center gap-1.5">
                      <BookOpen className="w-3.5 h-3.5 text-blue-600" />
                      <span>Attach Resources for Students</span>
                    </label>
                    <p className="text-[11px] text-slate-500">
                      Provide documentation, repositories, study guides, or links for students to reference.
                    </p>
                  </div>
                  <span className="text-[11px] font-semibold text-blue-600 px-2 py-0.5 rounded-full bg-blue-50 border border-blue-100">
                    {selectedResourceIds.length} attached
                  </span>
                </div>

                {/* Attached Resource Chips */}
                {selectedResourceIds.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 p-2.5 rounded-xl bg-slate-50 border border-slate-200/80 max-h-28 overflow-y-auto">
                    {selectedResourceIds.map((resId) => {
                      const resObj = hubResources.find((r) => r._id === resId);
                      const title = resObj ? resObj.title : 'Resource';
                      const type = resObj ? (resObj.type || 'link') : 'link';
                      return (
                        <span
                          key={resId}
                          className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white text-slate-800 text-xs font-medium border border-slate-200 shadow-2xs"
                        >
                          <span className="px-1.5 py-0.2 rounded bg-blue-100 text-blue-800 text-[10px] font-mono uppercase font-semibold">
                            {type}
                          </span>
                          <span className="truncate max-w-[200px] font-semibold text-slate-900">{title}</span>
                          <button
                            type="button"
                            onClick={() => setSelectedResourceIds((prev) => prev.filter((id) => id !== resId))}
                ...
                {/* Action Buttons: Browse Existing Resources / Add Custom Link */}
                <div className="flex flex-wrap items-center gap-2">
                  <Button
                    type="button"
                    variant="outline"
                    ...
                    className={`text-xs gap-1.5 h-8 ${showResourcePicker ? 'border-blue-600 bg-blue-50 text-blue-700' : ''}`}
                  >
                    <BookOpen className="w-3.5 h-3.5 text-blue-600" />
                    <span>{showResourcePicker ? 'Hide Resource Library' : `Select from Learning Hub (${hubResources.length})`}</span>
                  </Button>
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      setShowAddCustomLink(!showAddCustomLink);
                      setShowResourcePicker(false);
                    }}
                    className={`text-xs gap-1.5 h-8 ${showAddCustomLink ? 'border-blue-600 bg-blue-50 text-blue-700' : ''}`}
                  >
                    <Link2 className="w-3.5 h-3.5 text-emerald-600" />
                    <span>{showAddCustomLink ? 'Close Custom Link' : '+ Add Custom Link / Doc'}</span>
                  </Button>
                </div>
                ...
                {showResourcePicker && (
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2.5 animate-in fade-in">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs font-semibold text-slate-800">Learning Hub Resources:</span>
                      <input
                        type="text"
                        placeholder="Search resources..."
                        value={resourcePickerSearch}
                        onChange={(e) => setResourcePickerSearch(e.target.value)}
                        className="px-2.5 py-1 text-xs rounded-lg border border-slate-200 bg-white text-slate-800 focus:outline-none w-44"
                      />
                    </div>
                    ...
                    <div className="max-h-36 overflow-y-auto space-y-1.5 pr-1">
                      {hubResources
                        .filter((r) => {
                          if (!resourcePickerSearch.trim()) return true;
                          const q = resourcePickerSearch.toLowerCase();
                          return (
                        ...
                        .map((r) => {
                          const isAttached = selectedResourceIds.includes(r._id);
                          return (
                            <div
                              key={r._id}
                              onClick={() => {
                            ...
                            >
                              ...
                              <span className={`text-[11px] shrink-0 font-semibold ${isAttached ? 'text-blue-700' : 'text-slate-400'}`}>
                      ...
                      {hubResources.length === 0 && (
                        <p className="text-xs text-slate-400 text-center py-2">No learning resources uploaded yet.</p>
                      )}
                    </div>
                  </div>
                )}

                {/* Add Custom Link / Doc Form */}
                {showAddCustomLink && (
                  <div className="p-3 bg-emerald-50/40 rounded-xl border border-emerald-200/80 space-y-2.5 animate-in fade-in">
                    <div className="text-xs font-semibold text-emerald-900">
                      Create & Attach New Reference Link:
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                      <input
                        ...
                        className="sm:col-span-2 px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white text-xs text-slate-900 focus:outline-none"
                      />
                      <select
                        value={customLinkType}
                        onChange={(e) => setCustomLinkType(e.target.value)}
                        className="px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white text-xs text-slate-900 focus:outline-none font-medium"
                      >
                        <option value="link">Link / Webpage</option>
                        <option value="git_repo">GitHub Repo</option>
                        <option value="note">Note / Spec</option>
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white text-xs text-slate-900 focus:outline-none"
                      />
                      <Button
                        type="button"
                        size="sm"
                        disabled={creatingCustomLink}
                        onClick={handleCreateAndAttachCustomLink}
                        className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs h-8 shrink-0"
                      >
                        {creatingCustomLink ? 'Saving...' : 'Attach Resource'}
                      </Button>
                    </div>
                  </div>
                )}
